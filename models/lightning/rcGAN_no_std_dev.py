...
class rcGANNoStdDev(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if (self.global_step - 1) % self.args.pca_reg_freq == 0 and self.current_epoch >= 20:
            ...
            for n in range(gens_zm.shape[0]):
                ...
                w_loss += 1e-5 * w_obj.sum()  # 1e-3 for 25 iters

                gens_zm_det = gens_zm[n].detach()
                gens_zm_det[0, :] = x_zm[n, 0, :].detach()

                # 10 x 10, 10 x 128 128 x 10 10 x 10

                if self.current_epoch >= 50:
                    inner_product_mat = 1 / self.args.num_z_pca * torch.matmul(Vh, torch.matmul(torch.transpose(gens_zm_det.detach(), 0, 1) , torch.matmul(gens_zm_det.detach(), Vh.mT)))
                    sig_diff = (1 - 1 / (S ** 2) * torch.diag(inner_product_mat.clone().detach())) ** 2

                    sig_loss += 1e-3 * sig_diff.sum()
